...
`granite-lang-diagnostic-loom` is centered on compilers work in Julia. The technical target is: Create a Julia reference implementation for diagnostic workflows, centered on policy evaluation, deny and allow fixtures, and explainable decision traces.
